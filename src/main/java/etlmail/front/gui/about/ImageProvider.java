...
public class ImageProvider {
	...
	if (madame == null) {
	    madame = new JLabel();
	    new FetchImageWorker(madame, "http://feeds.feedburner.com/BonjourMadame?format=xml").execute();
	}
	return madame;
    }
}
...
class FetchImageWorker extends SwingWorker<BufferedImage, Void> {
    private static final Logger log = LoggerFactory.getLogger(FetchImageWorker.class);
    ...
    private List<SyndEntry> getFeedEntries(String url) throws IOException, ClientProtocolException, FeedException {
	...
	final String derniereEntree = entries.get(0).getDescription().getValue();
	final Document doc = Jsoup.parse(derniereEntree);
	final Elements image = doc.getElementsByTag("img");
	return image.attr("src");
    }

    private BufferedImage getImage(String url) throws IOException, ClientProtocolException {
    ...
    private InputStream get(String url) throws IOException, ClientProtocolException {
